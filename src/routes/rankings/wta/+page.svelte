<script lang="ts">
	import { wtaPlayers } from '$lib/data/wta';
	import PlayerRow from '$lib/components/PlayerRow.svelte';
</script>

<svelte:head>
	<title>WTA Rankings – Pro Tennis Statistics</title>
	<meta name="description" content="Current WTA rankings with 12-month points distribution for the top 10 players." />
</svelte:head>
...
<div class="space-y-6">
	...
	<div class="divide-y divide-border/40 rounded-2xl border border-border/60 bg-card/60">
		{#each wtaPlayers as player}
			<PlayerRow {player} />
		{/each}
	</div>

	<p class="text-center text-xs text-muted-foreground">
		Top 10 WTA players · Demo data for illustration purposes
	</p>
</div>
